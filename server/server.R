...
server <- function(input, output, session) {
  ...
  # Update the selectInput choices with the available event_IDs
  observeEvent(data_reactive(), {
    event_ids <- unique(data_reactive()$event_ID)
    updateSelectInput(session, "selected_event", choices = event_ids)
  })
}
